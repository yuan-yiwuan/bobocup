-- 0013: 投注倍数放宽 —— 所有比赛可 2 倍，自己主队的比赛最多 4 倍
-- 在 Supabase SQL Editor 运行。
--
-- 变更（相对 0010）：
--  - 任何比赛都可投 base 的 1~2 倍。
--  - 「自己主队的比赛」（主队踢主场或客场都行，胜/平/负任选）可投 base 的 1~4 倍。
--  - 即：3 倍及以上仅限自己主队的比赛。
--    小组赛 base=100（普通 100~200，主队 100~400）
--    淘汰赛 base=200（普通 200~400，主队 200~800）
...
begin
  if tg_op = 'INSERT'
    ...
    -- 下注额必须是 base 的 1~4 倍
    if new.stake not in (base, base * 2, base * 3, base * 4) then
      raise exception '下注额不合法';
    end if;

    -- 3 倍及以上（>base*2）只允许「自己主队的比赛」（主队在场，胜/平/负任选）
    if new.stake > base * 2 then
      ...
      if user_home is null or (user_home <> m_home and user_home <> m_away) then
        raise exception '只有自己主队的比赛才能投 3 倍以上';
      end if;
    end if;
  end if;
